...
async function safeExtractZip(zipPath, targetDir) {
    ...
    // 2. Fallback to 'jar' command (Java Archive Tool - Very robust for encoding issues)
    try {
        console.log('[ZIP] Falling back to JAR tool...');
        // jar xf <zipfile> (Run inside targetDir)
        await new Promise((resolve, reject) => {
            const jar = spawn('jar', ['xf', zipPath], { cwd: targetDir });

            jar.on('close', (code) => {
                if (code === 0) resolve();
                else reject(new Error(`jar process exited with code ${code}`));
            });
        ...
        console.log('[ZIP] JAR extraction success');
        return { success: true };
    } catch (jarError) {
        console.log(`[ZIP] JAR failed (${jarError.message}), falling back to AdmZip...`);
    }

    // 3. Fallback to Python 3 (Standard on Ubuntu/Debian, very robust zipfile lib)
    try {
        console.log('[ZIP] Falling back to Python 3...');
        // python3 -c "import zipfile,sys; zipfile.ZipFile(sys.argv[1]).extractall(sys.argv[2])" zipPath targetDir
        await new Promise((resolve, reject) => {
            const pyScript = `import zipfile,sys; zipfile.ZipFile(sys.argv[1]).extractall(sys.argv[2])`;
            const python = spawn('python3', ['-c', pyScript, zipPath, targetDir]);

            python.on('close', (code) => {
                if (code === 0) resolve();
                else reject(new Error(`python3 process exited with code ${code}`));
            });
        ...
        console.log('[ZIP] Python extraction success');
        return { success: true };
    } catch (pyError) {
        console.log(`[ZIP] Python failed (${pyError.message}), trying 7zip...`);
    }

    // 4. Fallback to 7zip (Extremely robust, handles almost all encodings)
    try {
        console.log('[ZIP] Falling back to 7zip...');
        // 7z x <zipfile> -o<targetDir> -y (extract, output, yes to all)
        await new Promise((resolve, reject) => {
            const sevenzip = spawn('7z', ['x', zipPath, `-o${targetDir}`, '-y']);

            sevenzip.on('close', (code) => {
                if (code === 0) resolve();
                else reject(new Error(`7z process exited with code ${code}`));
            });
        ...
        console.log('[ZIP] 7zip extraction success');
        return { success: true };
    } catch (sevenzipError) {
        console.log(`[ZIP] 7zip failed (${sevenzipError.message}), falling back to AdmZip...`);
    }

    // 5. Fallback to AdmZip with sanitization (Last resort)
    try {
